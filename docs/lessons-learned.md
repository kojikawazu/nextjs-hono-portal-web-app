...
issue #142 で `RESEND_API_KEY` をイメージ内 `.env` から外し、実行時に Secret Manager から注入する形にした。マージ後のデプロイで `next build` が `Missing API key` で失敗した。ビルド段階で止まったため、本番は旧 revision のまま動いており影響はない。

### 詳細

- 何が起きたか: PR #144 のマージで起動した deploy（run 35993674943）が Docker ビルドで失敗した。修正されるまで、main へのデプロイがすべて失敗する状態になった。
- なぜ起きたか（根本原因）: `mail.ts` がモジュールのトップレベルで `new Resend(process.env.RESEND_API_KEY)` を実行しており、Resend 4.x はキーが空だとコンストラクタで例外を投げる。`next build` はページデータの収集時に API ルートのモジュールを評価するため、**このキーはビルド時にも必要だった**。移行時には「キーは実行時にしか使わない」と判断し、ビルド時にモジュールが評価されることを確認していなかった。さらに、ユニットテストの Resend モックはキーの有無にかかわらず成功していたため、テストでも検出できなかった。
- 教訓 / 次からどうする:
  - **環境変数をビルド環境から外す前に、その変数を外した状態で `pnpm build` を実行する**（例: `RESEND_API_KEY= pnpm build`）。「実行時にしか使わない」は、トップレベルで参照していないことを確かめてから言う。
  - **外部 SDK のクライアントはトップレベルで生成せず、env を確認した後に生成する。** トップレベルでの生成は、import しただけで副作用と例外が起きる。
  - **モックは実物の失敗条件を再現する**（キーが空なら例外、など）。常に成功するモックは、その条件で起きる不具合を構造的に見逃す。
- 関連: issue #145 / issue #142 / PR #144

## 2026-09-24 手順書に残したシェルコマンドが zsh では引数を分割せず失敗した

### 概要

issue #142 の再開手順に、`for pair in "a b" ...; do set -- $pair; ... "$2"; done` 形式のコマンドを書いた。利用者の macOS 標準シェル（zsh）で実行すると、secret 名が空のまま `gcloud secrets versions add` が呼ばれ、2 回とも失敗した。書き込みは発生しておらず、実害はない。

### 詳細

- 何が起きたか: 秘密の値を Secret Manager へ投入するコマンドが `Failed to find attribute [secret]` で失敗した。作成した側は bash を前提に書いており、実行シェルでの動作確認をしていなかった。
- なぜ起きたか（根本原因）: bash はクォートしていない `$pair` を `IFS` で分割するが、**zsh は既定で分割しない**（`SH_WORD_SPLIT` がオフ）。そのため `set -- $pair` では `$1` に文字列全体が入り、`$2` が空になった。構文エラーにはならず、空の引数のままコマンドが実行された。
- 教訓 / 次からどうする:
  - **手順書・issue に残すシェルコマンドは、利用者が使うシェル（macOS なら zsh）で実行してから残す。** 実行していないコマンドは手順書に載せない。
  - **クォートなしの変数展開による分割に頼らない。** ループで値の組を回さず 1 行 1 操作で書くか、`${var%%:*}` / `${var##*:}` のように両シェルで同じ結果になるパラメータ展開を使う。
- 関連: issue #142 / [`manuals/terraform.md`](../manuals/terraform.md)

## 2026-09-24 リポジトリを grep した際に、Git 管理外の tfvars に入っていた API キーが AI セッションの出力に表示された

### 概要

Secret Manager への移行作業中、変数名の残りを探すために `grep -rn "resend_api_key" terraform/` を実行した。`.gitignore` 対象の `terraform/terraform.tfvars` にもヒットし、**Resend API キーの平文が AI エージェントのセッション出力に表示された**。Git や外部サービスへの流出はないが、キーは漏洩したものとして扱い、ローテーションする。

### 詳細

- 何が起きたか: 検索結果に tfvars の該当行がそのまま含まれ、Resend API キーとメールアドレスが会話ログに残った。影響は、このセッションのログを閲覧できる範囲。
- なぜ起きたか（根本原因）: **`.gitignore` で除外していても、作業ツリー上の検索対象からは外れない。** `grep -r` はファイルが Git 管理下かどうかを区別しない。検索の目的（参照箇所を探す）に対して、行の中身まで表示する `-n` の出力形式を選んだため、値ごと表示された。秘密を含むファイルが検索範囲内にあることを事前に考慮していなかった。
- 教訓 / 次からどうする:
  - **秘密を含みうるディレクトリ（`terraform/` や `.env` がある場所）を検索するときは、`--exclude='*.tfvars' --exclude='.env*'` を付けるか、`git grep`（Git 管理下だけを検索する）を使う。** 管理外のファイルを見る必要があるなら `-l` や `-c` でファイル名・件数だけを出す。
  - **値の一致確認はハッシュで行う**（`printf '%s' "$v" | shasum -a 256` 同士を比較する）。今回、Secret Manager と tfvars の照合はこの方法で、値を表示せずに行えた。
  - 表示してしまった場合は、ログの範囲にかかわらず**ローテーションを必須にする**。「推奨」に留めない。
- 関連: issue #142 / [`manuals/terraform.md`](../manuals/terraform.md)（ローテーション手順）
