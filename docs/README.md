...
> **コードパスの表記について**: 本リポジトリはモノレポ構成で、アプリ本体は **`front/`** に集約されている。各仕様書中の `src/…`・`__tests__/…`・`e2e/…` といったコードパスは、断りがない限り **`front/` 起点**（例: `src/app/...` は実体としては `front/src/app/...`）と読み替える。ディレクトリ全体像は [09-architecture-specification/structure.md](./09-architecture-specification/) を参照。
...
| 08 | [テスト仕様書](./08-test-specification/) | テスト戦略・テスト分類 |
