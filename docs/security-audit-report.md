...
> 上表は**監査実施時点（対応前）**の検出結果。対応後の状況は [最終結果](#最終結果) を参照（18件 → 2件）。
...
残存する8件の脆弱性を解消するため、Next.js 16へのアップグレードを実施した。
...
> 注: 本監査の実施時点（2026-01-31）のパッケージマネージャは npm。その後 pnpm へ移行している（`docs/11-tasks.md` の T-20）ため、現在このコマンドはそのままでは再現できない。
...
これらは上流パッケージ（Google Cloud Storage SDK）の依存関係であり、Google側の修正を待つ必要がある。

> 本監査の続きは [security-audit-report-2026-03.md](./security-audit-report-2026-03.md)（2026-03-21 実施）を参照。
