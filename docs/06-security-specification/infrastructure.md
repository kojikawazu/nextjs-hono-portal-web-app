...
| secret の読み取り権限 | `secretAccessor` は **secret 単位**で `cloud-run-sa` に付与（プロジェクト単位にしない）。ただし `cloud-run-sa` は他アプリ（`echo-blog-app` 等）と共有のため、**他アプリのサービスからも読める**。専用 SA への分離は issue #143 |
