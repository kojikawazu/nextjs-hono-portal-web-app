...
const aiUsageSeed = require('../../e2e/tests/mock/gcs-seed/it-bucket/aiusage.json');
...
describe('GCS Router (IT: fake-gcs-server 実結合)', () => {
    ...
    test('GET /aiusage - 正常系: エミュレータから取得', async () => {
        const res = await gcsRouter.fetch(new Request('http://localhost/aiusage'));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual(aiUsageSeed);
    });
    ...
    test('GET /aiusage - 異常系: 存在しないオブジェクト → 500', async () => {
        process.env.GCS_AIUSAGE_DATA_PATH = 'does-not-exist.json';
        const res = await gcsRouter.fetch(new Request('http://localhost/aiusage'));
        expect(res.status).toBe(500);
        expect(await res.json()).toEqual({ error: 'Failed to fetch data from GCS' });
    });
